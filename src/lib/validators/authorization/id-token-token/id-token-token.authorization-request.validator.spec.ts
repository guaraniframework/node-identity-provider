...
describe('ID Token Token Authorization Request Validator', () => {
  ...
  describe('name', () => {
    it('should have "id_token token" as its value.', () => {
      expect(validator.name).toEqual<ResponseTypeName>('id_token token');
    });
  });
  ...
  describe('constructor', () => {
    it('should instantiate a new ID Token Token Authorization Request Validator.', () => {
      expect(Object.getPrototypeOf(IdTokenTokenAuthorizationRequestValidator)).toBe(AuthorizationRequestValidator);
      expect(() => container.resolve(IdTokenTokenAuthorizationRequestValidator)).not.toThrow();
    });
  });

  describe('validate()', () => {
    let parameters!: IdTokenTokenAuthorizationRequest;

    const requestFactory = (data: Partial<IdTokenTokenAuthorizationRequest> = {}): HttpRequest => {
      removeNullishValues<IdTokenTokenAuthorizationRequest>(Object.assign(parameters, data));
    ...
    it.each(invalidNonces)('should throw when the provided parameter "nonce" is invalid.', async (nonce) => {
      ...
      expect(loggerMock.error).toHaveBeenCalledExactlyOnceWith(
        '[IdTokenTokenAuthorizationRequestValidator] Invalid parameter "nonce"',
    ...
    it('should return an ID Token Token Authorization Context.', async () => {
      ...
      await expect(validator.validate(request)).resolves.toMatchObject<IdTokenTokenAuthorizationContext>({
        parameters,
