...
describe('Code ID Token Authorization Request Validator', () => {
  ...
  describe('name', () => {
    it('should have "code id_token" as its value.', () => {
      expect(validator.name).toEqual<ResponseTypeName>('code id_token');
    });
  });
  ...
  describe('constructor', () => {
    it('should instantiate a new Code ID Token Authorization Request Validator.', () => {
      expect(Object.getPrototypeOf(CodeIdTokenAuthorizationRequestValidator)).toBe(CodeAuthorizationRequestValidator);
      expect(() => container.resolve(CodeIdTokenAuthorizationRequestValidator)).not.toThrow();
    });
  });

  describe('validate()', () => {
    let parameters!: CodeIdTokenAuthorizationRequest;

    const requestFactory = (data: Partial<CodeIdTokenAuthorizationRequest> = {}): HttpRequest => {
      removeNullishValues<CodeIdTokenAuthorizationRequest>(Object.assign(parameters, data));
    ...
    it.each(invalidNonces)('should throw when the provided parameter "nonce" is invalid.', async (nonce) => {
      ...
      expect(loggerMock.error).toHaveBeenCalledExactlyOnceWith(
        '[CodeIdTokenAuthorizationRequestValidator] Invalid parameter "nonce"',
    ...
    it('should return a Code ID Token Authorization Context.', async () => {
      ...
      await expect(validator.validate(request)).resolves.toMatchObject<CodeIdTokenAuthorizationContext>({
        parameters,
