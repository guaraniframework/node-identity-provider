...
describe('Code ID Token Token Authorization Request Validator', () => {
  ...
  beforeEach(() => {
    ...
    container.bind(CodeIdTokenTokenAuthorizationRequestValidator).toSelf().asSingleton();

    validator = container.resolve(CodeIdTokenTokenAuthorizationRequestValidator);
  });
  ...
  describe('name', () => {
    it('should have "code id_token token" as its value.', () => {
      expect(validator.name).toEqual<ResponseTypeName>('code id_token token');
    });
  });
  ...
  describe('constructor', () => {
    it('should instantiate a new Code ID Token Token Authorization Request Validator.', () => {
      expect(Object.getPrototypeOf(CodeIdTokenTokenAuthorizationRequestValidator)).toBe(
        CodeAuthorizationRequestValidator,
      );

      expect(() => container.resolve(CodeIdTokenTokenAuthorizationRequestValidator)).not.toThrow();
    });
  });

  describe('validate()', () => {
    let parameters!: CodeIdTokenTokenAuthorizationRequest;

    const requestFactory = (data: Partial<CodeIdTokenTokenAuthorizationRequest> = {}): HttpRequest => {
      removeNullishValues<CodeIdTokenTokenAuthorizationRequest>(Object.assign(parameters, data));
    ...
    it.each(invalidNonces)('should throw when the provided parameter "nonce" is invalid.', async (nonce) => {
      ...
      expect(loggerMock.error).toHaveBeenCalledExactlyOnceWith(
        '[CodeIdTokenTokenAuthorizationRequestValidator] Invalid parameter "nonce"',
    ...
    it('should return a Code ID Token Token Authorization Context.', async () => {
      ...
      await expect(validator.validate(request)).resolves.toMatchObject<CodeIdTokenTokenAuthorizationContext>({
        parameters,
